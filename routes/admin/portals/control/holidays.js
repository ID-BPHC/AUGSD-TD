...
router.post('/add', [
	...
	check('description').exists().withMessage('No Description').not().isEmpty().withMessage('No Description'),
	...
	holidaysModel.create({ date: req.sanitize(req.body.date), description: req.sanitize(req.body.description) }, function (err, result) {
		if (err) {
			return res.terminate(err);
		}
		return res.redirect('/admin/control/holidays');
