      ...
      if (val.admin) {
        projectsModel.find({}).exec((err, result) => {
          ...
          result.forEach((object, index, array) => {
            ...
            getdepartmentpromise.push(
              getProfessorDepartment(array[index].instructor)
            );
          });
  ...
  try {
    ...
    projectHeadModel.findOne(
      ...
      (err, result) => {
        ...
        } else return res.terminate("Could not find professor record.");
      }
    );
  } catch (err) {
    return res.terminate(err);
